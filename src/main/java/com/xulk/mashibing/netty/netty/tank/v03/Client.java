...
class ClientChannelInitializer extends ChannelInitializer<SocketChannel>{
    ...
    protected void initChannel(SocketChannel socketChannel) throws Exception {
        //socketChannel.pipeline().addLast(new ClientHandler());   引入TankMsgEncoder的处理
        socketChannel.pipeline().addLast(new TankMsgEncoder()).addLast(new ClientHandler());
...
class  ClientHandler extends ChannelInboundHandlerAdapter {
    ...
    */ 
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
       //跟上一个版本不同的是 上一个版本是直接写byteBuf
        //而这一个版本因为引入了tankMsgEncoder  所以可以直接写tankMsg，通过encoder进行编码 直接写出去（找到对应byteBuf 写入）
